...
static bool bl_poweron = true;
static int brightness_old = 0;
...
static int rk29_bl_update_status(struct backlight_device *bl)
{
	...
	int brightness = 0, brightness_cur;
...
//BL_CORE_DRIVER2 is the flag if backlight is into early_suspend.
    ...
    if (bl_poweron) {
        brightness_old = brightness = rk29_bl_info->min_brightness;
    }

#ifdef CONFIG_BATTERY_RK30_ADC
	if ((bl_check_battery_capacity() == 1) && (bl->props.brightness > 60))
	    brightness = 60;
	else if ((bl_check_battery_capacity() == 2) && (bl->props.brightness > 120))
	    brightness = 120;
	else if ((bl_check_battery_capacity() == 3) && (bl->props.brightness > 180))
	    brightness = 180;
#endif
...
//BL_CORE_DRIVER1 is the flag if backlight pwm is closed.
	...
	}else if((!(bl->props.state & BL_CORE_DRIVER1) && brightness != 0) || (bl_poweron)){
		bl_poweron = false;
		bl->props.state |= BL_CORE_DRIVER1;
    ...
    if ((brightness > brightness_old) || (brightness < brightness_old)) {
        brightness_cur = brightness_old;
        while (1) {
        	...
        	if (ref) {
        		divh = div_total*brightness_cur/BL_STEP;
        	} else {
        		divh = div_total*(BL_STEP-brightness_cur)/BL_STEP;
        	}
...
static void rk29_backlight_work_func(struct work_struct *work)
{
    ...
    brightness_old = rk29_bl_info->min_brightness;
