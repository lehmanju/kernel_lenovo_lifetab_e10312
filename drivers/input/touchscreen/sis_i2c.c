...
static int sis_gpio_status = 0;
static void sis_free_gpio(void)
{
	if (sis_gpio_status & (1<<0)) {
		gpio_free(TOUCH_RESET_PIN);
	}
	if (sis_gpio_status & (1<<1)) {
		gpio_free(GPIO_IRQ);
	}
	sis_gpio_status = 0;
}
...
static int initial_irq(void)
{
	...
	ret = gpio_request(TOUCH_RESET_PIN, "GPIO_RESET");	// ex. GPIO_133 for reset mode
	if (ret < 0)
	{
...
/*
    ...
    mdelay(30*1000);
	printk("%s(%d): ret=%d\n", __func__, __LINE__, ret);
	ret = sis_command_for_write(client, SIXTEEN_BYTE, wdata);
	printk("%s(%d): ret=%d\n", __func__, __LINE__, ret);
	if (ret < 0) {
		//goto err_input_dev_alloc_failed;
	}
*/
	...
	if (ret < 0) 
	{
		printk(KERN_ERR "sis_ts_probe: Unable to init gpio\n");
		goto err_input_register_device_failed;
	}
	else
	{
