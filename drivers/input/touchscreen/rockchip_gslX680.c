...
#define POWER_PORT			RK30_PIN2_PC2
#define IRQ_PORT			RK30_PIN4_PC2
#define WAKE_PORT			RK30_PIN6_PB0
...
{
    if (POWER_PORT != INVALID_GPIO) {
        if (gpio_request(POWER_PORT, "gslx680 power")) {
    		printk("pjf gpio_request(POWER_PORT) error\n");
            goto exit_alloc_gpio_failed;
        }
			...
			const struct i2c_device_id *id)
{
	...
	if (gslX680_chip_init() != 0) {
	    gslX680_chip_free();
		dev_err(&client->dev, "GSLX680 init chip failed\n");
	    goto error_mutex_destroy;
	}
	if (test_i2c(ts->client) < 0) {
	    gslX680_chip_free();
		dev_err(&client->dev, "GSLX680 init test i2c failed\n");
	    goto error_mutex_destroy;
	}
	...
	if (check_mem_data_init(ts->client) == -1) {
	    gslX680_chip_free();
		dev_err(&client->dev, "GSLX680 init mem failed\n");
	    goto error_mutex_destroy;
	}
