...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(u8* pbt_buf, int dw_lenth)
{
    ...
    int  i_is_new_protocol = 0, dtime = 5;
...
int fts_ctpm_auto_clb(void)
{
    ...
    msleep(200);
    buffer[0] = 0x40;
    fts_register_write(0, buffer);  
    mdelay(100);   //make sure already enter factory mode
    buffer[0] = 0x4;
    fts_register_write(2, buffer);  //write command to start calibration
    mdelay(300);
    for(i=0;i<100;i++)
    {
        fts_register_read(0, buffer, 1);
        if ( ((buffer[0]&0x70)>>4) == 0x0)  //return to normal mode, calibration finish
        {
            break;
        }
        mdelay(200);
    ...
    msleep(300);
    buffer[0] = 0x40;
    fts_register_write(0, buffer);  //goto factory mode
    mdelay(100);   //make sure already enter factory mode
    buffer[0] = 0x5;
    fts_register_write(2, buffer);  //store CLB result
    mdelay(300);
    buffer[0] = 0x0;
    fts_register_write(0, buffer); //return to normal mode 
    msleep(300);
...
static void ft5x0x_ts_resume(struct early_suspend *handler)
{
	...
	msleep(5);
	gpio_set_value(RK30_PIN6_PB0,GPIO_HIGH);
	msleep(200);
#if USE_POINT
	for(i=0; i<MAX_CONTACTS; i++) 
	{
...
/////////////////////////////////////////////////////////////////////////////
  ...
  if ((err < 0) || ((version[1] == 0x00) &&(version[2] == 0x00) &&(version[3] == 0x00))) {
