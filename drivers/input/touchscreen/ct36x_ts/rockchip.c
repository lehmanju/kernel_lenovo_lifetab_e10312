...
static int gpio_init_status = 0;
int ct36x_platform_get_resource(struct ct36x_ts_info *ct36x_ts)
{
	int err = -1;

	// Init PwrEn pin
	if (ct36x_ts->pwr != INVALID_GPIO) {
		err = gpio_request(ct36x_ts->pwr, "ct36x_ts_pwr");
		if ( err ) {
			return -EIO;
		}
		gpio_direction_output(ct36x_ts->pwr, 1);
		gpio_set_value(ct36x_ts->pwr, 1);
...
{
	if ((ct36x_ts->pwr != INVALID_GPIO) && (gpio_init_status & (1<<2))) {
	    gpio_free(ct36x_ts->pwr);
	}
