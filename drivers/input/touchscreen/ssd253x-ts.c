...
#define   ON_TOUCH_RST   RK30_PIN6_PB0
...
int deviceReset(struct i2c_client *client); 
...
{	
	...
	{
		ret = WriteRegister(	client,Reset[i].Reg,
...
*/
	...
	{
		pos = pos + cut_value;
		pos = SCREEN_MAX_X * pos / (XPOS_MAX + cut_value * 2);
		return pos;
	}
	else
	{
		pos = pos + cut_value;
		pos = SCREEN_MAX_Y * pos / (YPOS_MAX + cut_value * 2);
		return pos;
	}
}
#endif
static void ssd253x_ts_work(struct work_struct *work)
{
	...
	if(!Ssd_Timer_flag)
	{
		...
		if(Ssd_Timer2 < Ssd_Timer1)
       {
       		Ssd_Timer2 += 0xffff/1000;
		}
		if((Ssd_Timer2 - Ssd_Timer1) > 15)
		{
