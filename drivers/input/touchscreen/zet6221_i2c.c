...
#define TS_INT_GPIO		RK30_PIN4_PC2//S3C64XX_GPN(9)  /*s3c6410*/
//#define TS1_INT_GPIO        AT91_PIN_PB17 /*AT91SAM9G45 external*/
//#define TS1_INT_GPIO        AT91_PIN_PA27 /*AT91SAM9G45 internal*/
#define TS_RST_GPIO		RK30_PIN6_PB0//S3C64XX_GPN(10)
#define TS_PWR_GPIO		RK30_PIN2_PC2//S3C64XX_GPN(9)  /*s3c6410*/

#define TPINFO	1
#define X_MAX	1920//800//1024
#define Y_MAX	1152//480//576
...
extern int  zet622x_downloader(struct i2c_client * client);
...
int zet6221_charge_status = 0;
int zet6221_charge_status_old = 0;
...
void write_cmd_work()
{
    if (zet6221_charge_status != zet6221_charge_status_old) {
        if (zet6221_charge_status == 0) {
            zet6221_ts_charger_mode_disable();
        } else {
            zet6221_ts_charger_mode();
        }
        
        zet6221_charge_status_old = zet6221_charge_status;
    }
}
...
static void ts_late_resume(struct early_suspend *handler)
{
	...
	resetCount = 1;
	zet6221_charge_status_old = 0;
    return;
}

static u8 gpio_init_status = 0;

static int zet6221_free_io_port(struct zet6221_tsdrv *ts)
{
    ...
    if ((TS_PWR_GPIO != INVALID_GPIO) && (gpio_init_status & (1<<2))) {
        gpio_free(TS_PWR_GPIO);
    }

    gpio_init_status = 0;
}

static int zet6221_request_io_port(struct zet6221_tsdrv *ts)
{
	int result;
	...
	result = gpio_request(TS_INT_GPIO, "GPN"); 
	if (result) {
		goto gpio_io_fail;
	}
    ...
    if (TS_PWR_GPIO != INVALID_GPIO) {
    	result = gpio_request(TS_PWR_GPIO, "pwron"); 
    	if (result) {
    		goto gpio_io_fail;
    	}
...
static int __devinit zet6221_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	result = zet6221_request_io_port(zet6221_ts);
	if (result) {
	    printk( "[TS] zet6221_ts_probe: request io failed!\n");
		result = -ENOMEM;
		goto fail_alloc_mem0;
	}
	...
	if (!input_dev || !zet6221_ts) {
	    zet6221_free_io_port(zet6221_ts);
		result = -ENOMEM;
		goto fail_alloc_mem;
	}
