...
static int batt_table[2*BATT_NUM+6] =
{
	...
	3459, 3538, 3586, 3613, 3630, 3673, 3728, 3787, 3859, 3938, 4098,  //discharge
	3720, 3853, 3888, 3906, 3947, 4004, 4076, 4097, 4119, 4140, 4162	  //charge
};
...
extern int zet6221_charge_status;
#endif
...
#if defined (CONFIG_TOUCHSCREEN_ZET6221)
    zet6221_charge_status = charge_on;
#endif
...
#ifdef CONFIG_BACKLIGHT_RK29_BL
    if (rk_battery_get_status(bat) == 0) {
        if (bat->bat_capacity < 30) {
            if (bl_check_battery < 0)
                bl_check_battery = 0;
            bl_check_battery++;
    
            if (bl_check_battery > 10) {
                if (bl_check_flag != (bat->bat_capacity/10 + 1)) {
                    bl_check_flag = bat->bat_capacity/10 + 1;
...
{
    if ((gBatteryData->bat_status == POWER_SUPPLY_STATUS_NOT_CHARGING) && \
            (gBatteryData->bat_capacity < 30) && \
            (bl_check_flag == (gBatteryData->bat_capacity/10 + 1)))
        return bl_check_flag;
    else
        return 0;
}
